...
def SINBR_main(N_train,X_train,N_test,X_test,class_prior,indices_cnst,lr_N,lr_V,size_NN,K_val,file_name,opt_iters):
    ...
    pt13= tf.reduce_sum(((u1_sample - 1.0)*tf.log(theta10_sample+regul_)) - (tf.slice(nu_sample,[0,2],[K,1])*theta10_sample),axis=-1,keepdims=True)
    pt14= tf.reduce_sum(((u1_sample - 1.0)*tf.log(theta11_sample+regul_)) - (tf.slice(nu_sample,[0,3],[K,1])*theta11_sample),axis=-1,keepdims=True)
    ...
    pt16= (tf.reduce_sum(tf.log(tf.slice(nu_sample,[0,0],[K,2])+regul_),axis=-1,keepdims=True))*(tf.reduce_sum(u0_sample,axis=-1,keepdims=True))+(tf.reduce_sum(tf.log(tf.slice(nu_sample,[0,2],[K,2])+regul_),axis=-1,keepdims=True))*(tf.reduce_sum(u1_sample,axis=-1,keepdims=True))#modeified
    ...
    loss = tf.reduce_mean(scale*(log_H - log_P - log_P_prior + 0.2*loss_prior))
