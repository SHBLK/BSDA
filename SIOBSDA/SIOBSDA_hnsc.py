...
def SINBR_main(N_train,X_train,N_test,X_test,class_prior,indices_cnst,lr_N,lr_V,size_NN,K_val,file_name,opt_iters):
    ...
    sigp11_ = tf.get_variable("sig_p11",shape=[1,n_size_2[0]], dtype=tf.float32,initializer=tf.initializers.random_normal(mean=-5.0,stddev=0.1,seed=25))
    ...
    mu_p11 = tf.get_variable("mu_p11",shape=[1,n_size_2[0]], dtype=tf.float32,initializer=tf.initializers.random_normal(mean=0.0,stddev=0.1,seed=29))
    ...
    n_dta11 = tf.placeholder(tf.float32,[P[0],n_size_2[0]],name='data_n11')

    n_dta_test = tf.placeholder(tf.float32,[P[0],n_test],name='data_n_test')

    n_dta_valid = tf.placeholder(tf.float32,[P[0],n_size_1[1]+n_size_2[0]],name='data_n_valid')
    ...
    mu_theta10 = tf.slice(psi_sample,[0,psi_dim_phi+psi_dim_theta_each],[-1,psi_dim_theta_each])
    mu_theta11 = tf.slice(psi_sample,[0,psi_dim_phi+2*psi_dim_theta_each],[-1,psi_dim_theta_each])
    mu_u0 = tf.slice(psi_sample,[0,psi_dim_phi+3*psi_dim_theta_each],[-1,psi_dim_u_each])
    mu_u1 = tf.slice(psi_sample,[0,psi_dim_phi+3*psi_dim_theta_each+psi_dim_u_each],[-1,psi_dim_u_each])
    mu_nu = tf.slice(psi_sample,[0,psi_dim_phi+3*psi_dim_theta_each+2*psi_dim_u_each],[-1,psi_dim_nu])
    mu_b = tf.slice(psi_sample,[0,psi_dim_phi+3*psi_dim_theta_each+2*psi_dim_u_each+psi_dim_nu],[-1,psi_dim_b])
    mu_q = tf.slice(psi_sample,[0,psi_dim_phi+3*psi_dim_theta_each+2*psi_dim_u_each+psi_dim_nu+psi_dim_b],[-1,psi_dim_q])
    mu_c = tf.slice(psi_sample,[0,psi_dim_phi+3*psi_dim_theta_each+2*psi_dim_u_each+psi_dim_nu+psi_dim_b+psi_dim_q],[-1,psi_dim_c])
    mu_gamma = tf.slice(psi_sample,[0,psi_dim_phi+3*psi_dim_theta_each+2*psi_dim_u_each+psi_dim_nu+psi_dim_b+psi_dim_q+psi_dim_c],[-1,psi_dim_gamma])
    ...
    pt13= tf.reduce_sum(((u1_sample - 1.0)*tf.log(theta10_sample+regul_)) - (tf.slice(nu_sample,[0,1],[K,1])*theta10_sample),axis=-1,keepdims=True)
    pt14= tf.reduce_sum(((u1_sample - 1.0)*tf.log(theta11_sample+regul_)) - (tf.slice(nu_sample,[0,2],[K,1])*theta11_sample),axis=-1,keepdims=True)
    pt15= -Lab_num[0]*tf.reduce_sum(tf.lgamma(u0_sample),axis=-1,keepdims=True)-Lab_num[1]*tf.reduce_sum(tf.lgamma(u1_sample),axis=-1,keepdims=True)
    pt16= (tf.reduce_sum(tf.log(tf.slice(nu_sample,[0,0],[K,1])+regul_),axis=-1,keepdims=True))*(tf.reduce_sum(u0_sample,axis=-1,keepdims=True))+(tf.reduce_sum(tf.log(tf.slice(nu_sample,[0,1],[K,2])+regul_),axis=-1,keepdims=True))*(tf.reduce_sum(u1_sample,axis=-1,keepdims=True))
    ...
    log_P_prior = pt1+ pt2+ pt3+ pt4+ pt5+ pt6+ pt7+ pt8+ pt9 + pt10+ pt11+ pt13+ pt14+ pt15+ pt16 + pt17 +pt18+pt20+pt21
    ...
    log_P = tf.reduce_sum(tf.reduce_sum(tf.lgamma(tf.add(N_train_P00,tf.expand_dims(thetaphi00_sample,axis=-1))),axis=2),axis=1,keepdims=True)+\
        ...
        -n_size_1[0]*tf.reduce_sum(tf.lgamma(thetaphi00_sample),axis=1,keepdims=True)\
        -n_size_1[1]*tf.reduce_sum(tf.lgamma(thetaphi10_sample),axis=1,keepdims=True)-n_size_2[0]*tf.reduce_sum(tf.lgamma(thetaphi11_sample),axis=1,keepdims=True)+\
    ...
    train_op2 = tf.train.GradientDescentOptimizer(learning_rate=lr2).minimize(loss,var_list=[sigdirichlet_,sigtheta_00,\
        sigtheta_10,sigtheta_11,sigu_0,sigu_1,signu_,sigb_,sigq_,sigc_,siggamma_,mu_p00,mu_p10,mu_p11,\
            sigp00_,sigp10_,sigp11_])
...
#%%
    ...
    for i in range(opt_iters):
        _,cost=sess.run([train_op1,loss],{n_dta00:N_training_1[0],n_dta10:N_training_1[1],n_dta11:N_training_2[0],n_dta_test:N_test,n_dta_valid:N_train[1],lr:lr_N*(0.7**(i/opt_iters_red)),scale:min(1.0e-0,1.0)})

        if i<opt_iters_V:
            if (lr_V/lr_N) < 0.01:
                for dum_cnt in range(3):
                    _,cost_n,=sess.run([train_op2,loss],{n_dta00:N_training_1[0],n_dta10:N_training_1[1],n_dta11:N_training_2[0],n_dta_test:N_test,n_dta_valid:N_train[1],lr2:lr_V*(0.9**(i/opt_iters_red)),scale:min(1.0e-0,1.0)})
            else:
                _,cost_n,=sess.run([train_op2,loss],{n_dta00:N_training_1[0],n_dta10:N_training_1[1],n_dta11:N_training_2[0],n_dta_test:N_test,n_dta_valid:N_train[1],lr2:lr_V*(0.9**(i/opt_iters_red)),scale:min(1.0e-0,1.0)})
